...
np.array([28, 29, 11, 12, 30, 31, 62, 63])
...
MD_NAMES = list(MARKET_DESIGNS.keys())
...
ROW_YLIMS = [
    (-0.34, -0.15),    # row 0 — Planner Reward
...
def plot_lines(ax, ci, metric_key, data_cache, last_pct_only=False, lw=1.0):
    for md_name, keys in MARKET_DESIGNS.items():
        key  = keys[ci]
...
def make_figure(output_path='training_inset.pdf'):
    ...
    all_keys = {key for keys in MARKET_DESIGNS.values() for key in keys}
...
if __name__ == '__main__':
    make_figure(output_path='training_reward_all_planner_inset.pdf')
    print('Done.')