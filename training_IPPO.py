...
count_file_path = 'run_count_tax_ss_663.txt'
...
if __name__ == "__main__":
    ...
    root_directory = "/users_home/cmcc/jg24923/ray_results/PPO"

    # Define the prefix of the directories you want to delete
    prefix = "PPO_CM_EoM_ss_battery_663"
    ...
    base_path = "/work/cmcc/jg24923/CM_EoM_ss_battery_663/PPO"
    ...
    register_env("CM_EoM_ss_battery_663", env_creator)
    ...
    for agent_id in env.possible_agents_g:
        ...
        policies[policy_name] = spec
        policies_g[policy_name] = spec
    ...
    for agent_id in env.possible_agents_p:
        ...
        policies[policy_name] = spec
        policies_p[policy_name] = spec
 
    # ─────────────────────────────────────────────────────────────────────────
    # IPPO RLModule specs — SEPARATE actor and critic networks per agent.
    #
    # `vf_share_layers=False` is the switch that makes the catalog build TWO
    # independent encoders (policy + value), not just two heads on a shared
    # trunk. We set it explicitly because passing model_config without
    # vf_share_layers can silently flip encoders to shared on EnvRunners
    # (see ray-project/ray#58715).
    #
    # Sizing:
    #   - fcnet_hiddens      → encoder MLP size (applied to BOTH the policy
    #                          encoder AND the value encoder when not shared)
    #   - head_fcnet_hiddens → policy head AND value head MLP size
    # ─────────────────────────────────────────────────────────────────────────

    _model_cfg = {
    # — read by PPOCatalog (encoder + default vf_head) —
    "fcnet_hiddens":         [512, 512],   # encoder trunk; with vf_share_layers=False
                                           #   the critic gets its OWN [512, 512] encoder
    "fcnet_activation":      "relu",
    "head_fcnet_hiddens":    [64],         # size of PPO's default vf_head (critic head)
    "head_fcnet_activation": "relu",
    "vf_share_layers":       False,        # critic gets independent encoder

    # — read by your CustomPPOCatalog.get_action_dist_config() —
    "head_hidden":           64,           # per-action-dim head hidden size in the
                                           #   ResidualTrunkMultiHeadActor
    }
    ...
    config = (
        PPOConfig()
        .environment("CM_EoM_ss_battery_663")
        .api_stack(
        ...
        .rl_module(
            ...
            # model_config removed from here — it lives inside each RLModuleSpec above
        )
        .framework("torch")
        .learners(
        ...
        .multi_agent(
            ...
            algorithm_config_overrides_per_module={
                **{f"policy_{agent_id}": PPOConfig.overrides(
                    clip_param=0.1,
                ) for agent_id in env.possible_agents_g},
                **{f"policy_{agent_id}": PPOConfig.overrides(
                    clip_param=0.1,
    ...
    if run_count == 0:
        ...
        tuner = tune.Tuner(
            ...
            run_config=tune.RunConfig(
                ...
                storage_path="/work/cmcc/jg24923/CM_EoM_ss_battery_663",
